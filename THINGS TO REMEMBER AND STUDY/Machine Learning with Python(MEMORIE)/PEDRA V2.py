...
"""
VERSÂO COM 5 estrategias ao mesmo tempo
"""
...
"""
COM Q LEARNING INCLUSO:
"""
def player(prev_play, opponent_history=[], plb = ['S', 'R', 'S', 'R'], hist = [[] ,[] ,[], [], []], jogadas_hist = [], Q = [[0,0,0],[0,0,0],[0,0,0]]):
    ...
    if len(opponent_history) > 2: 
      ...
      for e in range(len(hist)):
        ...
        for x in range(len(playlast10)):
          if ideal_response[opponentlast10[x]] == playlast10[x]:
            acertos += 1
        chance_strage[e] = (acertos - (1 if ideal_response[hist[e][-1]] == prev_play else 0) )/len(playlast10) if acertos != 0 else 0        
      #Chances
      #quincy
